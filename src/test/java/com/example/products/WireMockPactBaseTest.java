...
@EnableWireMock({
    @ConfigureWireMock(name = "wiremock-service-name", property = "wiremock.server.url", stubLocation = "wiremock", configurationCustomizers = {
        WireMockPactBaseTest.class })
})
...
public class WireMockPactBaseTest implements WireMockConfigurationCustomizer {
  ...
  static WireMockPactExtension WIREMOCK_PACT_EXTENSION = new WireMockPactExtension(
      WireMockPactConfig.builder() //
          ...
          .setProviderDefaultValue(System.getenv().getOrDefault("PACT_PROVIDER", "pactflow-provider")) //
